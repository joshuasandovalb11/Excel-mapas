      ...
      >
        {filteredAndSortedRoutes.length === 0 ? (
          ...
          filteredAndSortedRoutes.map((r) => {
            const isSelected = selectedRouteId === r.id_ruta;
            ...
            return (
              <button
                key={r.id_ruta}
                onClick={() => onSelectRoute(r.id_ruta)}
                disabled={isLoading}
                className={`w-full text-left py-1.5 2xl:py-2 px-2 2xl:px-3 rounded-md 2xl:rounded-lg border transition-all duration-200 group relative overflow-hidden flex flex-col gap-0.5 2xl:gap-1
                  ${
                    isSelected
                      ? 'bg-gray-50 border-blue-600 shadow-xs ring-1 ring-blue-600/10'
                      : 'bg-white border-gray-200 shadow-xs hover:border-gray-300 hover:bg-gray-50'
                  }
                `}
              >
                {isSelected && (
                  <div className="absolute top-0 left-0 w-1 h-full bg-blue-600 rounded-l-md 2xl:rounded-l-lg"></div>
                )}

                <div className="flex justify-between items-center pl-1 2xl:pl-1.5 w-full">
                  <span
                    className={`text-[10px] 2xl:text-[11px] font-semibold truncate pr-2 ${
